...
Client.prototype.get = function(key, callback) {
  ...
  this.seq++;
  var request = makeRequestBuffer(opcodes.get, key, '', '', this.seq);
  this.perform(key, request, _makeGetResponseHandler(callback, logger));
};
...
// _value_ and _flags_ are both `Buffer`s. If the key is not found, the
// callback is invoked with null for both arguments and no error.
Client.prototype.getq = function(key, callback) {
  var logger = this.options.logger;
  this.seq++;
  var request = makeRequestBuffer(opcodes.getq, key, '', '', this.seq);
  this.perform(key, request, _makeGetResponseHandler(callback, logger));
};

function _makeGetResponseHandler (callback, logger) {
  return function onGetResponse (err, response) {
    if (err) {
...
// GET MULTI
//
// Issues parallel get requests for multiple keys
Client.prototype.getMulti = function(keys, callback) {
  ...
  // handle multiple responses and cache them until we receive all.
  function handlePartial (err, results) {
    if (err) {
      errors.push(err);
    }
  ...
  for (var i = 0; i < keys.length; i++) {
    memcached.getq(keys[i], handlePartial);
  }
};
